...
working_path = "100k_fit_and_evaluate_experiments/"

simulator_microservice_url = 'http://3.122.52.113:25041/'

# set this equal to the number of threads in the simulator virtual machine
no_of_threads = 72

result_file_name = "100k_fit_and_evaluate_experiments/fit_eval_results.csv"
	...
	while(True):
		...
		if len(jobs) > 0:
			...
			with open(job, "r") as f:
				no_of_layers, no_of_channels, activation_name, model_checkpoint_file = f.readline().split(",")
			...
			simulation_test_Xy_filename = "100k_fit_and_evaluate_experiments/simulation_Xy_%d_%d_%s.csv" % (no_of_layers, no_of_channels, activation_name)
			...
			with open(result_file_name, "a") as f:
				f.write("%d,%d,%s,%s,%f,%f" % (no_of_layers, no_of_channels, activation_name, model_checkpoint_file, model_avg_error, simulation_model_avg_error))
				f.write("\n")
				f.close()
