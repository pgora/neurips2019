...
if __name__ == '__main__':
	...
	file_to_save_results = "permuted_topologies_0_kipf.csv"
	with open(file_to_save_results,"a") as f:
		for i in numbers_of_transpositions:
			...
			test_and_batch_losses = train_eval.train_model(nn_output, nn_input, targets, optimizer, 30000, batch_iterator, X_test, y_test, "trained_networks/random_model_tmp_kipf.ckpt", 1000, verbose=True)
			test_loss = test_and_batch_losses[-1][0]
			model_avg_error, actual_vs_predicted = train_eval.evaluate_model_on_a_dataset("trained_networks/random_model_tmp_kipf.ckpt", nn_output,nn_input, X_test, y_test, y_scaler)
